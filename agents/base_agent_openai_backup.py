...
class BaseAgent(ABC):
    ...
    def __init__(self, agent_name: str, system_prompt: str = None):
        ...
        # Initialize LLM with fallback for quota exceeded
        self.llm = None
        self.chain = None
        try:
            ...
            logger.warning(f"OpenAI initialization failed for {agent_name}: {e}. Using fallback mode.")
    ...
    def _invoke_llm(self, input_text: str, **kwargs) -> str:
        ...
        if self.chain is None:
            logger.warning(f"{self.agent_name}: OpenAI unavailable, using fallback logic")
            return f"FALLBACK_MODE: {input_text[:100]}"
        ...
        try:
            ...
            logger.warning(f"{self.agent_name}: Using fallback mode due to LLM error")
            return f"FALLBACK_MODE: {input_text[:100]}"
